...
- [x] M5.4 — DAL + TanStack Query + poll/SSE on the **generated client** + **Zod** at the boundary — (⟵ M5.3, ⛔ **BE/M5.1 + BE/M5.C** cross-repo) — [state-and-polling] — done: typed, server-auth in the DAL, live data
  ...
  > **Live-swap ledger:** dashboard `/v1/inferences` → **LIVE** (M2.5b, zero FE changes) · defend `/v1/remediations?inference_id=` → **LIVE** (FE PR #53; honest same-scale **0.86→0.21 verified in-browser**; states proven/within_noise/cant_break) · trust `/v1/eval/*` → **green-lit** (BE PR #54, FE swap pending) · attribution `/v1/inferences/{id}` → **BLOCKED on the id gap** (needs `AttributeRead.id` + detail read 501→real; the same `id` also lets the dashboard hand an `inference_id` to /defend — the FE `?inference=` seam is ready and waiting).
- [x] M5.6 — tests: **Vitest + RTL + MSW** (async RSC → Playwright) + **Playwright** E2E vs preview env — (⟵ M5.4, M5.5, ⛔ **BE deployed** for E2E) — [10-testing] — done: green in CI — **Vitest 71 + Playwright E2E 33 green (PRs #14, #21); axe WCAG-AA sweep (light+dark) caught + fixed 9 contrast bugs. CI E2E needs Clerk repo secrets (runs locally now); live-data E2E awaits the deployed backend. Deeper layers (journeys/invariant/property/edge) added as bonus hardening.**
