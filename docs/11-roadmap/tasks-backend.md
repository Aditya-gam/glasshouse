...
- [ ] M2.5 — per-user scoring (raw → calibrated) — (⟵ M2.4) — [per-user-scoring] — done: calibrated stored, raw hidden — **M2.5a done (PR #41): per-user scoring → calibrated_reliability keyed by the bare ATTACK engine_version + signal + N + bucket (matches the inference; fixed M2.4's scoring-version key mismatch); fail-closed to NULL on a map miss, never raw. M2.5b next = flip GET /v1/inferences (calibrated AttributeRead[] cards, taxonomy severity seed mig 0008, OpenAPI update) = the DASHBOARD-LIVE milestone. Reliability band degenerate {lo=hi=point} until noise_std (M3.4) widens it — FE renders a point.**
