...
- [x] M2.5 — per-user scoring (raw → calibrated) — (⟵ M2.4) — [per-user-scoring] — done: calibrated stored, raw hidden — **M2.5a done (PR #41): per-user scoring → calibrated_reliability keyed by the bare ATTACK engine_version + signal + N + bucket (matches the inference; fixed M2.4's scoring-version key mismatch); fail-closed to NULL on a map miss, never raw. M2.5b done (PR #42): GET /v1/inferences serves calibrated AttributeRead[] cards — **DASHBOARD LIVE**. `evidence` = count-summary placeholder (full attribution later). Reliability band degenerate {lo=hi=point} until noise_std (M3.4) widens it — FE renders a point.**
...
- [ ] M5.2 — `/v1` routers + poll + SSE — (⟵ M5.1) — [endpoints/*] — done: 202+run_id, authz, no-IDOR — **partial: POST/GET/poll/SSE live (built early for FE-enablement); list/cancel + pagination remain. Contract drifted since M5.1 (idempotency_key body→header, SSE live) → FE regenerates the client. READ-flip progress (each flips once its **calibrated** shape exists — the dashboard needs calibrated reliability, not raw): GET /v1/inferences → live (M2.5b) · GET /v1/remediations → live (M3.8) · GET /v1/eval/results + /v1/eval/calibration → live (PR #54; **public no-auth** reads of the latest eval run via `get_public_session` + `latest_eval_run_id()`/`latest_eval_engine_version()` SECURITY DEFINER helpers; empty arrays until a real `run_eval` executes; curve = equal-weight macro-avg, sample-weighting is a tracked follow-up). GET /v1/inferences/{id} → live (PR #55; tenant-scoped RLS+ownership, 404 no-IDOR; in-query decrypt w/ bound keys; **Art.9 fail-closed at the SQL source** — value+reasoning+evidence-rationale masked without `art9_inference` consent; +`AttributeRead.id` additive; adversarial review caught + fixed a real high-sev un-gated `rationale_ct` leak). REMAINING: GET /v1/runs list + cursor pagination (cancel live). **TRACKED security follow-up:** the LIVE dashboard LIST read GET /v1/inferences still decrypts art9 values **un-gated** (same leak class the detail just closed) — apply the read-time art9 gate there too. Compose fixes still open (surfaced by FE live-testing): worker in the default profile · stale venv volume · **api `APP_DATABASE_URL=…@db:5432`** (the RLS app-role engine defaults to localhost in-container → 500s every request path; FE fixed locally, uncommitted — **BE to own + commit**). Surfaced by FE PR #42.**
