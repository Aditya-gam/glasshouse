...
| **GeoNames** | geocoding (location normalization, `M1.7b`) | **inferred place names only** (e.g. "Seattle") — no user content, no identity |
...
The cloud LLM/VLM are the only **content** egress; **GeoNames** is a narrow secondary egress (inferred place names only — see `trust-boundaries.md`); in **local/dev** (Ollama) there is **no** external content egress. Adding/changing a sub-processor updates this list + the DPA (traceability).
